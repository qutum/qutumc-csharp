...
public class TestLexer : IDisposable
{
	...
	{
		Throw(() => new Lexer("A=[ab]+ [bc]"), "A.2 and A.1 .*repeat");
		Throw(() => new Lexer("A=a[ab]+|b[bc]+ b"), "A.2 and A.1 .*repeat");
	}
